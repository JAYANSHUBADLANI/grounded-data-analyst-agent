...
One caveat specific to this model: Vertex AI's OpenAI-compatible endpoint doesn't accept fully disabling Gemini's internal reasoning for 2.5-series models, `minimal` is the lowest setting it allows, and testing before this run showed even a trivial tool call still burns real hidden reasoning tokens under it. That's baked into the token and cost numbers above, it isn't overhead this project's own design introduced.
...
3. **This run used Gemini 2.5 Flash-Lite, not the Llama-3.3-70B this agent was designed around.** See Evaluation Results above for why. The agent and baseline code are provider-agnostic through `llm_client.py`, so the numbers here reflect this specific model, not a property of the architecture, a Llama run would very plausibly land on different numbers, particularly latency, since Llama has no equivalent hidden reasoning overhead.
4. **Vertex AI's reasoning_effort couldn't be set below "minimal" for this model**, so every call here carries some hidden reasoning-token cost that a non-reasoning model wouldn't have. It's a real, measured part of the cost and latency figures above, not hidden or excluded.
